...
if((p0_3==0)&&(fkset==1)){
	...
	if((mode==0)&&(mag==0)){						//待機モードで方向センサーが0ならば
		...
		if((gr > 100 + margin_wait)&&(gr >= grb + margin_wait + back_spd_bdline)){	//待機位置よりも前に居る場合(遠い)
...
/*************************************************************デジタル***/
	...
	if((mode!=25)&&(mode!=50)&&(gr < gra )&&(gr > gra - 30)&&(p9!=0xff)){	//中立点エリアでデジタル赤外線センサーが本能していたら
		if(p9_0==0)mode=70;
		if(p9_1==0)mode=71;
		if(p9_2==0)mode=72;
		if(p9_3==0)mode=73;
	...
	if((mode!=25)&&(mode!=50)&&(gr < gra - 30)&&(gr > grc )&&(p9!=0xff)){	//通常動作エリアでデジタル赤外線センサーが反応していたら
		if(p9_0==0)mode=80;
		if(p9_1==0)mode=81;
		if(p9_2==0)mode=82;
		if(p9_3==0)mode=83;
