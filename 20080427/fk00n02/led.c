...
if((p0_3==0)&&(fkset==1)){
	...
	if((mode==0)&&(mag==0)){						//待機モードで方向センサーが0ならば
		...
		if((gr > 100 + margin_wait)&&(gr < 100 + margin_wait + back_spd_bdline)){	//待機位置よりも前に居る場合(近い)
		...
		if((gr > 100 + margin_wait)&&(gr >= 100 + margin_wait + back_spd_bdline)){	//待機位置よりも前に居る場合(遠い)
...
/************************************************************ 中立点***/ 
	...
	if((mode==20)&&(gr > 160)) mode=29;
	if((mode==29)&&(gr < 160-30)) mode=20;
	if((mode==29)&&(irr+irc+irl < 700)) mode=0;
...
/*************************************************************デジタル***/
	...
	if((mode!=25)&&(mode!=50)&&(gr > 160 )&&(p9!=0xff)){			//立ち入り禁止エリアでデジタル赤外線センサーが反応していたら
		if(p9_0==0)mode=60;
		if(p9_1==0)mode=61;
		if(p9_2==0)mode=62;
		if(p9_3==0)mode=63;
	...
	if((mode!=25)&&(mode!=50)&&(gr < 160 )&&(gr > 160 - 30)&&(p9!=0xff)){	//中立点エリアでデジタル赤外線センサーが本能していたら
		if(p9_0==0)mode=70;
		if(p9_1==0)mode=71;
		if(p9_2==0)mode=72;
		if(p9_3==0)mode=73;
	...
	if((mode!=25)&&(mode!=50)&&(gr < 160 - 30)&&(gr > 100 -5 )&&(p9!=0xff)){	//通常動作エリアでデジタル赤外線センサーが反応していたら
		if(p9_0==0)mode=80;
		if(p9_1==0)mode=81;
		if(p9_2==0)mode=82;
		if(p9_3==0)mode=83;
	...
	if((mode!=25)&&(mode!=50)&&(gr < 100-5)&&(p9!=0xff)){			//待機位置以下でデジタル赤外線センサーが反応していたら
		if(p9_0==0)mode=90;
		if(p9_1==0)mode=91;
		if(p9_2==0)mode=92;
		if(p9_3==0)mode=93;
	...
	if((mode>=60)&&(p9==0xff))mode=0;		//デジタル赤外線が反応しているモード(mode=60～96)で赤外線センサーが反応していなかったら
...
//蹴る
	...
	if((mode==50)&&(ball_catch==0)){
		if((ball_catch_pt<160+5)&&(irl*2 > irc+irr)){		//左が一番大きい場合は
			pwm(64,64,32,32,64);
			p2=0x09;
			p1=0x09;
		}

		if((ball_catch_pt<160+5)&&(irc*2 > irr+irl)){		//中央が一番大きい場合は
			pwm(32,32,32,32,64);
			p2=0x09;
			p1=0x69;
		}

		if((ball_catch_pt<160+5)&&(irr*2 > irc+irl)){		//右が一番大きい場合は
			pwm(64,64,32,32,64);
			p2=0x09;
			p1=0x60;
		}

		if(ball_catch_pt>=160+5){
			p2=0xff;
			p1=0xff;
		}
